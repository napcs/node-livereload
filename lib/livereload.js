...
(function() {
  var EventEmitter, Server, chokidar, clientScriptPath, defaultExclusions, defaultExts, defaultPort, fs, http, https, path, protocol_version, url, ws,
  ...
  Server = (function(superClass) {
    ...
    function Server(config1) {
      ...
      if ((base8 = this.config).socketPath == null) {
        base8.socketPath = '/livereload';
      }
    ...
    if (!config.server) {
      requestHandler = function(req, res) {
        if (url.parse(req.url).pathname === '/livereload.js') {
          ...
          return fs.createReadStream(clientScriptPath).pipe(res);
        }
      };
      config.server = config.https != null ? https.createServer(config.https, requestHandler) : http.createServer(requestHandler);
    }
